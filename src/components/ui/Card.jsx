...
const Card = ({ title, description, hoverText, icon: Icon, delay = 0, bgImage = null }) => {
    ...
    const getMobile = (path) => {
        if (!path) return path;
        // Handle versioned filenames: insert _mobile before _vN suffix
    ...
    return (
        ...
        >
                    ...
                    <div className="absolute inset-0 z-0">
                        <picture>
                            ...
                            <img
                                ...
                                loading="lazy"
                                decoding="async"
                                className="w-full h-full object-cover opacity-45 group-hover:opacity-60 group-hover:scale-105 transition-all duration-200"
            ...
            <div className="relative z-10 h-full flex flex-col justify-start">
                <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[#D4AF37]/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-200" />
