...
export default function DeProofIndex() {
    return (
        <>
            <SEO
                title="Nachweis"
                description="Inbetriebnahmedisziplin, Release-Kontrolle und Nachvollziehbarkeit — gezeigt, nicht behauptet."
                keywords="Beweis, Inbetriebnahme, Release-Richtlinie, Audit-Protokoll"
            />
            ...
            <section className="relative h-[80vh] flex flex-col justify-center items-center text-center px-6 overflow-hidden">
                ...
                <m.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="relative z-10 max-w-4xl mx-auto">
                    <div className="flex items-center justify-center gap-3 mb-6">
                        ...
                        <span className="text-xs tracking-[0.2em] text-[#D4AF37] uppercase">Nachweisbasis</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-light tracking-wide text-white mb-8">Nachweis</h1>
                    <p className="text-xl md:text-2xl text-white/50 font-light leading-relaxed max-w-2xl mx-auto mb-12">
                        Inbetriebnahmedisziplin, Release-Kontrolle und Nachvollziehbarkeit — gezeigt, nicht behauptet.
                    </p>
                </m.div>
            </section>
            ...
            <section className="py-24 px-6 bg-[#0A0A0A]">
                <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-8">
                    {[
                        { title: "Inbetriebnahme", desc: "Abnahmetests, die beweisen, dass sich das System auch unter Ausfällen korrekt verhält.", link: "/de/proof/commissioning", icon: ShieldCheck },
                        { title: "Release-Richtlinie", desc: "Kontrollierte Updates und Rollback-Disziplin.", link: "/de/proof/release-policy", icon: RefreshCw },
                        { title: "Audit-Beispiel", desc: "Jede Aktion mit Korrelations-IDs aufgezeichnet.", link: "/de/proof/audit-log", icon: FileSearch }
                    ].map((card, idx) => (
                        <SmartLink key={idx} to={card.link} className="block group">
                            <m.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }} className="p-8 border border-white/5 bg-[#0C0C0C] h-full relative overflow-hidden group-hover:border-[#D4AF37]/30 transition-all duration-200">
                                ...
                                <div className="flex items-center gap-2 text-xs uppercase tracking-widest text-[#D4AF37] opacity-60 group-hover:opacity-100 transition-opacity">
                                    Nachweis ansehen <ArrowRight className="w-3 h-3" />
                                </div>
                            </m.div>
                        </SmartLink>
                    ))}
